...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"indexaddress\"\n"
            "\nReturn information about the given Index address.\n"
            "\nArguments:\n"
            "1. \"indexaddress\"     (string, required) The Index address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"indexaddress\", (string) The Index address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Index address and we have full public key:
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"znsync\": true|false,             (boolean) if znode data is synced\n"
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "     [\n"
            "       \"key\"    (string) Index address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"indexaddress\"  (string, required) The Index address to use for the signature.\n"
